...
namespace ImGuiNET.Unity
{
    ...
    sealed class ImGuiRendererMesh : IImGuiRenderer
    {
        ...
        {
            //io.BackendRendererName = new NullTerminatedString(*x);                            // setup renderer info and capabilities
        ...
        unsafe void UpdateMesh(ImDrawDataPtr drawData)
        {
...
#pragma warning restore
            ...
            for (int n = 0, nMax = drawData.CmdListsCount; n < nMax; ++n)
            {
                ...
                for (int i = 0, iMax = drawList.CmdBuffer.Size; i < iMax; ++i, ++subOf)
                {
                    ...
                    if (prevTextureId != drawCmd.GetTexID())
                        _properties.SetTexture(_texID, _texManager.GetTexture((int)(prevTextureId = drawCmd.GetTexID())));
        ...
        public static unsafe byte[] GetStringBytes(string str)
        {
            byte[] buffer = new byte[str.Length];
            fixed (byte* b = buffer)
            {
                Marshal.StructureToPtr(str, new IntPtr(b), false);
